...
const RelaxOnHero = () => {
  ...
  useEffect(() => {
    ...
    if (spotlight) {
      spotlight.style.background = `radial-gradient(circle at ${currentPos.x}px ${currentPos.y}px, rgba(8, 75, 91, 1), transparent 35%)`
    }
  }, [currentPos])
  ...
  return (
    <section className="h-screen w-full relative overflow-hidden text-zinc-200 font-sans" onMouseMove={handleMouseMove}>
      ...
      <div
        ...
        style={{
          background: `radial-gradient(circle at ${currentPos.x}px ${currentPos.y}px, rgba(8, 75, 91, 1), transparent 35%)`,
        }}
      />
      ...
      <div className="relative z-20 flex flex-col items-start justify-center h-full px-4 sm:px-6 md:px-16 space-y-4 sm:space-y-6 w-full max-w-full overflow-x-hidden">
        <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-5xl font-bold break-words leading-tight">
          Relax On - Mobile Massage Application
        </h1>
        <br />
